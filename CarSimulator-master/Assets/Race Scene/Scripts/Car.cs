...
//			}
	...
	void FixedUpdate() {
		...
		float activeThrottle = (Throttle * Engine.GetTorque (Rigidbody2D) * 1.5f) * (Engine.GearRatio * Engine.EffectiveGearRatio);
		if (Throttle < 0) {
			activeThrottle /= 2;
		}
		...
		//Debug.Log ("vel: " + AbsoluteVelocity + ", Limit: " + speedLimit);
		if (AbsoluteVelocity > speedLimit) {
			Velocity = Velocity / (AbsoluteVelocity / speedLimit);
		}
...
//		GUI.contentColor = new Color (1f, 1f, 1f, 0.8f);
		...
		GUI.Label (new Rect(Screen.width * 0.44f, 3f, Screen.width * 0.3f, 27f), "Durability: " + Mathf.Round(Durability*10)/10 + "/" + DurabilityMax + " - " + Mathf.Round(1000*Durability/DurabilityMax)/10 + "%");
